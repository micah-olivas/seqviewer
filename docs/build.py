...
/* A command is a line of text and wraps like one, with its continuations
   indented so the wrap is not read as a second command.  Output does not
   wrap: it is fixed-pitch, and a histogram folded at the window edge is no
   longer a histogram, so that one scrolls instead. */
.run .cmd {
  background: var(--panel); color: var(--ink);
  border: 1px solid var(--rule);
  white-space: pre-wrap; overflow-wrap: anywhere;
  padding-left: 2.4rem; text-indent: -1.5rem;
}
...
<section class="tool">
  ...
  <div class="body">
    ...
    output.</p>
    <div class="run">
      <pre class="cmd">$ <b>uv tool install</b> --python 3.13 \
    'seqviewer[cli,fast,plot] @ git+https://github.com/micah-olivas/seqviewer'</pre>
    </div>
